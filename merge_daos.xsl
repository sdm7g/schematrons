...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="component_xml" select="resolve-uri(concat('ts_components/',$basename),$basedir)"></xsl:param>
    ...
    <xsl:template match="ead:did">
        ...
        <xsl:variable name="myid" >
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:message>No did/@id or ead/@id <xsl:value-of  select="saxon:path()"/></xsl:message>
        ...
        <xsl:copy>
            ...
            <xsl:if test="parent::ead:archdesc" ><xsl:apply-templates select="node()" /></xsl:if>
            ...
            <xsl:choose>
                <xsl:when test="count($component) > 1">
                    ...
                    <xsl:message>  WARNING: More than one component matching id: <xsl:value-of select="$myid"/> : <xsl:value-of select="saxon:path()" /></xsl:message>
                    <xsl:for-each select="$component">
                        <xsl:element name="dao">
                            <xsl:attribute name="xlink:type">simple</xsl:attribute>
                            <xsl:attribute name="xlink:href" select="concat($ts_component_url, ./component-id )" />
                            <xsl:attribute name="xlink:title" select=" substring(concat( 'Tracksys:[',/level,']: ',  normalize-space(./desc )),1,255)"/>
                        </xsl:element>
                ...
                <xsl:when test="count($component) = 1">
                    <xsl:element name="dao">
                        <xsl:attribute name="xlink:type">simple</xsl:attribute>
                        <xsl:attribute name="xlink:href" select="concat($ts_component_url, normalize-space($component/component-id) )" />
                        <xsl:attribute name="xlink:title" select=" substring(concat( 'Tracksys:[', $component/level,']: ', normalize-space($component/desc) ), 1, 255)" />
                    </xsl:element>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:message>No component found for <xsl:value-of select="$myid"/> : <xsl:value-of select="saxon:path()"/></xsl:message>
                </xsl:otherwise>
            </xsl:choose>
            
            <xsl:if test="not(parent::ead:archdesc)" ><xsl:apply-templates select="node()" /></xsl:if>
